...
{
    if((K_STA & userInput->pressedKey) != 0)
    {
...
{
	// const char* strPong = I18n::getText(I18n::getInstance(), kStringPong);

	// FontSize textSize = Printer::getTextSize(strPong, "VirtualBoyExt");

	// Printer::text(strPong, (__SCREEN_WIDTH >> 4) - (textSize.x >> 1), 12, "VirtualBoyExt");
}
